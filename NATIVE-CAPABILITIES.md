...
## Voice input fix (v7.5.9-fix1)

Android voice input now uses the native `SpeechRecognizer` instead of relying on WebView `SpeechRecognition` permission handling. The Java bridge requests `RECORD_AUDIO` only when the AI microphone button is tapped and sends partial/final transcription back to the web UI.

The web UI keeps the existing browser SpeechRecognition fallback for normal browsers.
